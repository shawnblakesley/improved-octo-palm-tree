/* =========================================================================
   ...
   Differences from the base course shell:
   - Every lesson carries its own inline quiz (auto-graded multiple choice)
     plus optional "reflect" prompts with model answers.
   - Lessons support a list of exercises, not just one.
   - Progress and best scores are stored per lesson id and per phase id.
   ========================================================================= */
...
(function () {
  ...
  /* Every lesson gets a quiz; lookups work for both lesson and phase ids. */
  function quizById(id) {
    var list = allLessons();
    for (var i = 0; i < list.length; i++) {
      if (list[i].lesson.id === id) {
        return { kind: "lesson", questions: list[i].lesson.quiz || [] };
      }
    }
    var p = phaseById(id);
    if (p && p.quiz) return { kind: "phase", questions: p.quiz };
    return null;
  }
  ...
  function phaseStats(phase) {
    ...
    var quizzes = phase.lessons.filter(function (l) { return state.quizzes[l.id]; }).length;
    var quiz = state.quizzes[phase.id] || null;
    var pct = total ? Math.round((done / total) * 100) : 0;
    return { total: total, done: done, quizzes: quizzes, quiz: quiz, pct: pct };
  }
  ...
  function overallStats() {
    ...
    var quizzesTaken = lessons.filter(function (x) { return state.quizzes[x.lesson.id]; }).length;
    var pct = total ? Math.round((done / total) * 100) : 0;
    return { total: total, done: done, pct: pct, quizzesTaken: quizzesTaken };
  }
  ...
  function renderSidebar(activePhaseId) {
    ...
    html += '<div class="brand-mark">' + esc(COURSE.mark || "GD") + "</div>";
    ...
    html += '<div class="nav-label">Units</div>';
  ...
  function renderDashboard() {
    ...
    html += '<div class="stat"><strong>' + s.quizzesTaken + "/" + s.total + "</strong><span>Lesson quizzes</span></div>";
    html += '<div class="stat"><strong>' + s.pct + "%</strong><span>Complete</span></div>";
    html += "</div>";
    if (next) {
      html += '<a class="btn primary big" href="#/phase/' + next.phase.id + '">' +
        (s.done === 0 ? "Start Unit 1" : "Continue: " + esc(next.lesson.title)) + "</a>";
    } else {
      html += '<p class="all-done">You have completed every lesson. Revisit any unit, or start the capstone.</p>';
    }
    html += "</header>";

    html += '<div class="callout"><strong>Target outcome</strong><p>' + esc(CURRICULUM.target) + "</p></div>";

    html += '<h2 class="section-title">Units</h2>';
  ...
  function renderPhaseCard(p) {
    ...
    html += '<span class="chip">Unit ' + p.num + "</span>";
    ...
    html += '<span>' + st.quizzes + " / " + st.total + " quizzes</span></div>";
    html += "</a>";
    return html;
  }
  ...
  function renderPhase(phaseId) {
    ...
    var html = "";
    html += '<div class="crumbs"><a href="#/">Overview</a> <span>/</span> Unit ' + p.num + "</div>";
    html += '<header class="phase-head">';
    html += '<p class="eyebrow">Unit ' + p.num + " &middot; " + esc(p.duration) + "</p>";
    ...
    html += renderVideos("Watch: Unit " + p.num, phaseVideos);
    ...
    if (prev) html += '<a class="btn ghost" href="#/phase/' + prev.id + '">&larr; Unit ' + prev.num + "</a>";
    else html += '<a class="btn ghost" href="#/">&larr; Overview</a>';
    if (next) html += '<a class="btn ghost" href="#/phase/' + next.id + '">Unit ' + next.num + " &rarr;</a>";
    html += "</nav>";

    main.innerHTML = html;
  }

  /* Shared quiz markup: one form per lesson and one per phase review. */
  function renderQuizForm(scopeId, questions) {
    var html = '<form class="quiz-form" data-quiz-form="' + esc(scopeId) + '">';
    questions.forEach(function (q, qi) {
      html += '<fieldset class="question" data-qindex="' + qi + '">';
      html += '<legend><span class="qnum">Q' + (qi + 1) + "</span>" + esc(q.q) + "</legend>";
      q.options.forEach(function (opt, oi) {
        var id = "q-" + scopeId + "-" + qi + "-" + oi;
        html += '<label class="option" for="' + id + '"><input type="radio" id="' + id + '" name="q-' + scopeId + "-" + qi + '" value="' + oi + '"><span>' + esc(opt) + "</span></label>";
      });
      html += '<div class="explain" hidden></div>';
      html += "</fieldset>";
    });
    html += '<div class="quiz-actions">';
    html += '<button type="button" class="btn primary" data-action="submit-quiz" data-quiz-id="' + esc(scopeId) + '">Check answers</button>';
    html += '<button type="button" class="btn ghost" data-action="retake-quiz" data-quiz-id="' + esc(scopeId) + '">Clear</button>';
    html += "</div>";
    html += "</form>";
    return html;
  }
  ...
  function renderLesson(phase, lesson, index, shownUrls) {
    ...
    if (state.quizzes[lesson.id]) {
      html += '<span class="chip good" title="Best quiz score">' + quizScoreLabel(state.quizzes[lesson.id]) + "</span>";
    }
    ...
    var exercises = lesson.exercises || (lesson.exercise ? [lesson.exercise] : []);
    exercises.forEach(function (ex, ei) {
      var exId = lesson.id + "-ex" + ei;
      var doneEx = !!state.exercises[exId];
      html += '<div class="exercise' + (doneEx ? " done" : "") + '">';
      html += '<div class="exercise-top"><span class="chip accent">Exercise</span><h4>' + esc(ex.title) + "</h4></div>";
      html += '<p class="task">' + esc(ex.task) + "</p>";
      if (ex.hint) html += '<details class="hint"><summary>Hint</summary><div>' + esc(ex.hint) + "</div></details>";
      html += '<label class="check ex-check"><input type="checkbox" data-action="mark-exercise" data-id="' + exId + '"' + (doneEx ? " checked" : "") + '><span>Mark exercise complete</span></label>';
      html += "</div>";
    });

    if (lesson.quiz && lesson.quiz.length) {
      html += '<section class="lesson-quiz">';
      html += '<h3 class="col-title">Lesson quiz <span class="quiz-count">' + lesson.quiz.length + " questions</span></h3>";
      html += renderQuizForm(lesson.id, lesson.quiz);
      html += "</section>";
    }

    if (lesson.reflect && lesson.reflect.length) {
      html += '<section class="lesson-reflect">';
      html += '<h3 class="col-title">Reflect</h3>';
      lesson.reflect.forEach(function (r, ri) {
        html += '<details class="hint reflect-item"><summary>' + esc(r.q) + "</summary>";
        html += '<div><strong>Model answer.</strong> ' + esc(r.model) + "</div></details>";
      });
      html += "</section>";
    }
  ...
  function renderQuizCta(p) {
    if (!p.quiz || !p.quiz.length) return "";
    var st = phaseStats(p);
    var html = '<section class="quiz-cta">';
    html += '<div><h3>Unit ' + p.num + " review quiz</h3>";
  ...
  function renderQuiz(phaseId) {
    var p = phaseById(phaseId);
    if (!p || !p.quiz) { renderNotFound(); return; }
    var saved = state.quizzes[p.id] || null;

    var html = "";
    html += '<div class="crumbs"><a href="#/">Overview</a> <span>/</span> <a href="#/phase/' + p.id + '">Unit ' + p.num + "</a> <span>/</span> Review quiz</div>";
    html += '<header class="phase-head"><p class="eyebrow">Knowledge check</p><h1>' + esc(p.title) + " review</h1>";
    if (saved) html += '<p class="goal">Best so far: <strong>' + quizScoreLabel(saved) + "</strong></p>";
    html += "</header>";

    html += renderQuizForm(p.id, p.quiz);
    html += '<p class="quiz-back"><a class="btn ghost" href="#/phase/' + p.id + '">&larr; Back to unit</a></p>';
  ...
  /* ---------------------------- actions ---------------------------- */
  function gradeQuiz(scopeId) {
    var quiz = quizById(scopeId);
    if (!quiz || !quiz.questions.length) return;
    var form = document.querySelector('[data-quiz-form="' + scopeId + '"]');
    if (!form) return;
    ...
    questions.forEach(function (q, qi) {
      ...
      var selected = form.querySelector('input[name="q-' + scopeId + "-" + qi + '"]:checked');
    ...
    var prev = state.quizzes[scopeId];
    var record = { score: score, total: questions.length, date: Date.now() };
    if (!prev || score >= prev.score) {
      state.quizzes[scopeId] = record;
      save();
    }

    var summary = form.querySelector(".quiz-actions");
    var msg = form.querySelector(".quiz-result");
    if (!msg) {
      msg = document.createElement("div");
      msg.className = "quiz-result";
      form.insertBefore(msg, summary);
    }
    msg.innerHTML = '<strong>' + score + " / " + questions.length + "</strong> correct" +
      (unanswered ? " &middot; " + unanswered + " unanswered" : "") +
      ". " + (score === questions.length ? "Perfect." : "Review the explanations, then retake if you like.");

    setTopbar();
    var hash = window.location.hash;
    var m = hash.match(/^#\/phase\/([^/]+)/);
    renderSidebar(m ? m[1] : null);
    if (m) updatePhaseChrome(m[1]);
  }

  function clearQuiz(scopeId) {
    var form = document.querySelector('[data-quiz-form="' + scopeId + '"]');
    if (!form) return;
    form.reset();
  ...
  document.addEventListener("click", function (e) {
    ...
    if (action === "submit-quiz") { gradeQuiz(t.getAttribute("data-quiz-id")); }
    else if (action === "retake-quiz") { clearQuiz(t.getAttribute("data-quiz-id")); }
  ...
  function updatePhaseChrome(phaseId) {
    var p = phaseById(phaseId);
    if (!p) return;
    ...
    if (card) {
      card.classList.toggle("complete", pst.pct === 100);
      var foot = card.querySelector(".card-foot");
      if (foot) {
        foot.innerHTML = "<span>" + pst.done + " / " + pst.total + " lessons</span>" +
          "<span>" + pst.quizzes + " / " + pst.total + " quizzes</span>";
      }
    }
    var cta = document.querySelector(".quiz-cta .quiz-best");
    if (cta && pst.quiz) cta.innerHTML = "<strong>" + quizScoreLabel(pst.quiz) + "</strong><span>best score</span>";
  }
